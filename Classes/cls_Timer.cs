...
namespace timebot.Classes
{
    ...
    public class ReminderTimer
    {
        ...
        public static async Task<System.Timers.Timer> CreatePublicTimer (SocketChannel chan, nextevent t)
        {
            ...
            tmr.Elapsed += async (object sender, ElapsedEventArgs e) =>
            {
                await PublicReminderCallBack (chan, t);
            };

            return tmr;
        }
        public static async Task RegisterTimer (SocketChannel chan, nextevent e, bool is_private)
        {
            ...
            {
                var t = await CreatePublicTimer (chan, e);

                t.Start ();
                Program._publictimers.Add (t);
            }
        ...
        public static async Task PublicReminderCallBack (SocketChannel chan, nextevent t)
        {
            if (chan == null) return;

            var emb = Helper.ObjToEmbed (t, "name");

            await Program._client.GetGuild (435921918152146945).GetTextChannel (chan.Id).SendMessageAsync ("Timer Has Elapsed", false, emb, null);
        }
    }
}